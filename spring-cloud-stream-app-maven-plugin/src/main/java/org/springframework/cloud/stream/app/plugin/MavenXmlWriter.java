...
public class MavenXmlWriter {
	...
	public static void main(String[] args) {
		...
		//Writer writer = new StringWriter();
		//MavenXpp3WriterEx pomWriter = new MavenXpp3WriterEx();
		//Model model = new Model();
		//model.setDependencies(Arrays.asList(dep));
		//pomWriter.write(writer, model);
		//System.out.println(writer.toString());

		for (Dependency dependency : Arrays.asList(dep, dep, dep)) {
			System.out.print(toXml(dependency));
		}

		Plugin plugin = new Plugin();
		plugin.setGroupId("org.springframework.cloud.stream.app.plugin");
		plugin.setArtifactId("spring-cloud-stream-app-maven-plugin");
		plugin.setVersion("2.0.0.BUILD-SNAPSHOT");

		System.out.print(toXml(plugin));
	}
}
